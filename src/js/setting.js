var currentCantoId = window.currentCantoId; 

if (!currentCantoId) {
    const urlParams = new URLSearchParams(window.location.search);
    currentCantoId = urlParams.get('canto') || 'global';
    window.currentCantoId = currentCantoId;
}
...
{ 
    ...
    accion: (val) => {
        if (window.guardarNotaPersonalCanto) {
            let urlLimpia = val.trim();
            
            // Si no está vacío y no tiene http, se lo ponemos
            if (urlLimpia !== "" && !/^https?:\/\//i.test(urlLimpia)) {
                urlLimpia = `https://${urlLimpia}`;
            }

            // Actualizamos el cuadro de texto para que el usuario vea el cambio
            const input = document.getElementById('set-url-personal');
            if (input) input.value = urlLimpia;

            // IMPORTANTE: Guardamos urlLimpia, NO val
            window.guardarNotaPersonalCanto(urlLimpia, 'url');
        }
    }
},
